...
impl KeyPair {
    ...
    pub fn decrypt(
        ...
        match profile_key_struct::ProfileKeyStruct::from_M4(
            ciphertext.E_B2 - (self.b * ciphertext.E_B1),
        ) {
            Err(_) => Err(DecryptionFailure),
            Ok(decrypted_profile_key) => {
                if ciphertext.E_B1 == self.calc_E_B1(decrypted_profile_key) {
                    Ok(decrypted_profile_key)
                } else {
...
mod tests {
    ...
    fn test_profile_key_encryption() {
        ...
        let profile_key = profile_key_struct::ProfileKeyStruct::new(TEST_ARRAY_16);
